...
function ensureClass(state, cls) {
  if (!state.classes[cls]) {
    state.classes[cls] = {
      ...
      // три независимых набора времен (уроки/продлёнка/полдник)
...
async function handleScheduleBusesUpload(env, token, msg, state, cls, caption, file_id) {
  ...
  // ИНАЧЕ — РАСПИСАНИЕ УРОКОВ (по умолчанию)
  ...
  switch (cmd) {
    ...
    case "/diag": {
      ...
      const lines = [
        ...
        `• автобусы (городские): ${rec.bus_file_id ? "есть ✅" : "нет"}, подвоз (школьный): ${rec.shuttle_file_id ? "есть ✅" : "нет"}`,
