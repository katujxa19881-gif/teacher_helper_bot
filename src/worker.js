...
function ensureClass(state, cls) {
  if (!state.classes[cls]) {
    state.classes[cls] = {
      ...
      shuttle_file_id: null, shuttle_caption: null, // подвоз
...
async function handleCommand(env, token, msg, state) {
  ...
  return true;
      ...
      // 5) сохраняем + оповещаем
state.classes[cls].pickup_times = mapping;
await saveState(env, state);

// краткое подтверждение в личку/группу
await sendToSameThread("sendMessage", token, msg, {
  text: `Готово. ${cls}: ${Object.entries(mapping).map(([k,v])=>`${k}=${v}`).join(", ")}`
});
...
// постим «на неделю» в привязанные чаты
{
  ...
  for (const chatId of targets) {
    await sendSafe("sendMessage", token, {
      chat_id: chatId,
      text: `Обновлено время (уроки, ${cls}):\n${table}`
    });
  }
}
