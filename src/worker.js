...
function ensureClass(state, cls) {
  if (!state.classes[cls]) {
    state.classes[cls] = {
      ...
      pickup_times: null, // во сколько забирать после уроков
...
function isLessonQuery(t){
  // запросы про уроки/занятия/предметы
...
function isScheduleLessonsQuery(t) {
  ...
  if (/расписан\w*\s*(урок|занят)/i.test(t)) return true;

  // «какие завтра уроки», «какие предметы», «что за уроки»
  if (/(какие|что за)?\s*(сегодня|завтра)?\s*(урок(и)?|занят(ия)?|предмет(ы)?)/i.test(t)) return true;

  // Короткие формы: «уроки?», «занятия?», «предметы?»
  if (/^(уроки|занятия|предметы)\b/i.test(t)) return true;

  // Иначе — не расписание уроков
  return false;
}
...
function isShuttleQuery(t) {
  // ЯВНО исключаем любые упоминания уроков/предметов/расписания,
  // чтобы запросы про уроки НИКОГДА не попадали в «подвоз»
  if (/(урок|занят|предмет|расписан)/i.test(t)) return false;

  // Требуем «подвоз» или связку «школьн.*автобус»
  return /\bподвоз\b/i.test(t) || /школьн\w*\s*автобус\w*/i.test(t);
}

/* область времени из текста (уроки/продлёнка/полдник) */
function scopeFromText(t) {
  const n = normalize(t);
  if (/(продленк|продлёнк|гпд)/.test(n)) return "aftercare";
  if (/(полдник|полденик)/.test(n)) return "snack";
  return "main";
}
...
function resolveTimeNatural(state, msg, freeText, teacherName) {
  ...
  const scope = scopeFromText(freeText || "");
...
async function cmdStart(token, chatId) {
  const text = [
    ...
    "/diag [КЛАСС] — короткая диагностика",
    "",
...
async function handleMediaFromTeacher(env, token, msg, state) {
  ...
  // Иначе — попытка как расписание/звонки/автобусы/подвоз (автопубликация)
...
if (/(расписани.*звонк|когда перемена|во сколько звонок)/i.test(t)) {
  ...
  if (/((когда|во сколько).*(заканчива|конча).*(урок|пара)|во сколько звонок|расписани.*звонк)/.test(t)) {
    const cls = pickClassFromChat(state, msg.chat.id) || "1Б";
    const rec = state.classes[cls] || {};
    if (rec.bells_file_id) {
  ...
  if (/(во сколько|когда).*(забир|забрать|забирать)/.test(t) || /(до скольки|во сколько).*(продленк|продлёнк|полдник)/.test(t)) {
  ...
  switch (cmd) {
    ...
    case "/pickup_set": {
      ...
      if (parts[1]) {
        ...
        else if (/^полдник/i.test(t2)) scope = "snack";
      }
    ...
    case "/teach": {
      ...
      const [, pat, ans] = m; state.teach = state.teach || []; state.teach.push({ pat: pat.trim(), ans: ans.trim() });
    ...
    case "/media_del": {
      ...
      const topic = (m[0] || "").trim(); const which = (m[1] || "").trim(); const cls = parseClassFrom(m.slice(2).join(" ") || "");
    ...
    case "/diag": {
      const cls = parseClassFrom(args || "") || (pickClassFromChat(state, msg.chat.id) || "1Б");
      ensureClass(state, cls);
      const rec = state.classes[cls];
      const lines = [
        `Диагностика для ${cls}:`,
