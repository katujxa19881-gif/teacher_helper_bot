...
// экспорт FAQ, пересылка неизвестных вопросов учителю, toggle пересылки, листинг/удаление/добавление FAQ.
// Хранилище: Cloudflare KV (KV_BOT)
...
function bestFaqAnswer(state, question) {
  if (!state.faq || !state.faq.length) return null;
  const scored = state.faq.map((item, idx) => ({ idx, item, s: scoreMatch(question, item.kw || []) }));
  scored.sort((a, b) => b.s - a.s);
  const top = scored[0];
  if (!top || top.s === 0) return null;
  if (top.s < 5) return null; // порог уверенности
  return top.item;
}

function listCategories(state) {
  const cats = new Set();
  for (const f of state.faq || []) if (f.cat) cats.add(f.cat);
  return Array.from(cats).sort();
}

/* -------------------- Keyboards -------------------- */
function kbCategories(cats) {
  // InlineKeyboard для категорий FAQ
  const rows = cats.map((c) => [{ text: `📚 ${c}`, callback_data: `faq_cat::${c}` }]);
...
function kbFaqItems(items, page = 0, perPage = 8, cat = "") {
  ...
  const rows = slice.map((it, i) => [
    { text: `${start + i + 1}. ${it.q.substring(0, 32)}…`, callback_data: `faq_show::${start + i}` },
  ]);
  const nav = [];
...
async function cmdSchedule(token, msg, state, args) {
  // 1) угадать класс по привязке текущего чата
  let cls = null;
  for (const [k, v] of Object.entries(state.classes)) {
    if (v.general_chat_id === msg.chat.id || v.parents_chat_id === msg.chat.id) {
      cls = k;
      break;
    }
  }
  // 2) если личка — можно указать класс аргументом
  if (!cls && msg.chat.type === "private") {
    const found = parseClassFrom(args);
    if (!found) {
      await tg("sendMessage", token, { chat_id: msg.chat.id, text: "Укажите класс: /schedule 5А" });
      return;
    }
    cls = found;
  }
  if (!cls) {
    await tg("sendMessage", token, {
      chat_id: msg.chat.id,
      text: "Этот чат не привязан к классу. Выполните /link_general 5А или /link_parents 5А.",
    });
    return;
  }
  const rec = state.classes[cls];
  if (!rec || !rec.schedule_file_id) {
    await tg("sendMessage", token, { chat_id: msg.chat.id, text: `Для ${cls} расписание ещё не загружено.` });
    return;
  }
  await tg("sendPhoto", token, {
    chat_id: msg.chat.id,
...
async function cmdAsk(env, token, msg, state, args) {
  const q = args || "";
  if (!q) {
    await tg("sendMessage", token, { chat_id: msg.chat.id, text: "Напишите вопрос после команды. Пример: /ask Когда начинаются уроки?" });
    return;
  }
  const hit = bestFaqAnswer(state, q);
  if (hit) {
    await tg("sendMessage", token, { chat_id: msg.chat.id, text: `Ответ:\n${hit.a}` });
    return;
  }
  if (state.forward_unknown_to_teacher && state.teacher_id) {
    await tg("sendMessage", token, {
      chat_id: state.teacher_id,
      text: `Вопрос от ${msg.from?.first_name || "родителя"} (${msg.chat.id}):\n${q}`,
    });
  }
  await tg("sendMessage", token, {
    chat_id: msg.chat.id,
    text: "Пока не нашёл готового ответа. Я передал вопрос учителю. Вы получите ответ в чате 🙌",
  });
}
...
async function cmdFaq(token, msg, state) {
  ...
  if (!faqs.length) {
    await tg("sendMessage", token, { chat_id: msg.chat.id, text: "FAQ пока пуст. Админ может добавить через /faq_add" });
    return;
  }
  ...
  // если категорий нет — показать первые пункты списком с кнопками
  await tg("sendMessage", token, {
    chat_id: msg.chat.id,
    text: "Частые вопросы:",
...
async function cmdFaqList(token, chatId, state) {
  const faqs = state.faq || [];
  if (!faqs.length) {
    await tg("sendMessage", token, { chat_id: chatId, text: "FAQ пуст." });
    return;
  }
  const out = faqs
    .map((x, i) => `${i + 1}. ${x.q}${x.cat ? ` [${x.cat}]` : ""}`)
    .join("\n");
  // Telegram ограничивает длину ~4096 символов — режем при необходимости
  for (let i = 0; i < out.length; i += 3500) {
    await tg("sendMessage", token, { chat_id: chatId, text: out.slice(i, i + 3500) });
  }
}
...
async function cmdFaqAdd(env, token, msg, state, args) {
  ...
  if (!isTeacher) {
    await tg("sendMessage", token, { chat_id: msg.chat.id, text: "Только учитель может добавлять FAQ." });
    return;
  }
  // Формат: /faq_add Вопрос | Ответ | ключ1, ключ2 | категория
  const parts = args.split("|").map((s) => s.trim());
  if (parts.length < 2) {
    await tg("sendMessage", token, { chat_id: msg.chat.id, text: "Формат: /faq_add Вопрос | Ответ | ключ1, ключ2 | категория" });
    return;
  }
  const q = parts[0];
  const a = parts[1];
...
async function cmdFaqDel(env, token, msg, state, args) {
  const isTeacher = state.teacher_id && state.teacher_id === msg.from.id;
  if (!isTeacher) {
    await tg("sendMessage", token, { chat_id: msg.chat.id, text: "Только учитель может удалять FAQ." });
    return;
  }
  const idx = parseInt(args, 10);
  if (!state.faq || isNaN(idx) || idx < 1 || idx > state.faq.length) {
    await tg("sendMessage", token, { chat_id: msg.chat.id, text: "Укажите номер записи: /faq_del 2" });
    return;
  }
...
async function cmdForwardUnknown(env, token, msg, state, args) {
  const isTeacher = state.teacher_id && state.teacher_id === msg.from.id;
  if (!isTeacher) {
    await tg("sendMessage", token, { chat_id: msg.chat.id, text: "Доступ только учителю." });
    return;
  }
  const v = (args || "").trim().toLowerCase();
  if (v !== "on" && v !== "off") {
    await tg("sendMessage", token, { chat_id: msg.chat.id, text: "Используйте: /forward_unknown on|off" });
    return;
  }
...
async function handleCallback(env, token, cb, state) {
  ...
  if (kind === "faq_cat") {
    ...
    if (!items.length) {
      await tg("answerCallbackQuery", token, { callback_query_id: cb.id, text: "В этой категории пока ничего" });
      return;
    }
  ...
  if (kind === "faq_nav") {
    ...
    await tg("editMessageReplyMarkup", token, {
      chat_id: chatId,
      message_id: cb.message.message_id,
      reply_markup: kbFaqItems(items, Math.max(0, page), 8, cat),
    });
  ...
  if (kind === "faq_show") {
    ...
    // Тут нужен контекст списка. Для простоты возьмём общий список (или последний отрисованный).
    // В реальной продакшн-версии можно хранить в KV «последний список» по message_id.
    const all = state.faq || [];
    if (!all[idx]) {
      await tg("answerCallbackQuery", token, { callback_query_id: cb.id, text: "Элемент не найден" });
      return;
    }
    const item = all[idx];
    await tg("answerCallbackQuery", token, { callback_query_id: cb.id });
...
async function handleCommand(env, token, msg, state) {
  ...
  switch (cmd) {
    ...
    case "/iam_teacher":
      ...
      return true;
    case "/link_general":
      await cmdLink(token, msg, state, args, "link_general");
      await saveState(env, state);
      return true;
    case "/link_parents":
      await cmdLink(token, msg, state, args, "link_parents");
      await saveState(env, state);
      return true;
    case "/schedule":
      await cmdSchedule(token, msg, state, args);
      return true;
    case "/ask":
      await cmdAsk(env, token, msg, state, args);
      return true;
    case "/faq":
      await cmdFaq(token, msg, state);
      return true;
    case "/faq_list":
      await cmdFaqList(token, chatId, state);
      return true;
    case "/faq_export":
      await cmdFaqExport(token, chatId, state);
      return true;
    case "/faq_add":
      await cmdFaqAdd(env, token, msg, state, args);
      await saveState(env, state);
      return true;
    case "/faq_del":
      await cmdFaqDel(env, token, msg, state, args);
      await saveState(env, state);
      return true;
    case "/forward_unknown":
      await cmdForwardUnknown(env, token, msg, state, args);
      await saveState(env, state);
      return true;
    default:
      return false;
...
export default {
  async fetch(request, env) {
    ...
    // Инициализация вебхука: POST /init
    if (url.pathname === "/init" && request.method === "POST") {
      if (!env.BOT_TOKEN || !env.PUBLIC_URL) return NO(400, "Need BOT_TOKEN and PUBLIC_URL");
      const res = await tg("setWebhook", env.BOT_TOKEN, { url: `${env.PUBLIC_URL}/webhook/${env.BOT_TOKEN}` });
    ...
    if (url.pathname === `/webhook/${env.BOT_TOKEN}` && request.method === "POST") {
      ...
      if (update.message?.text) {
        const handled = await handleCommand(env, env.BOT_TOKEN, update.message, state);
        if (handled) return OK();
      }

      // Фото расписания
      if (update.message?.photo?.length) {
        await handlePhotoFromTeacher(env, env.BOT_TOKEN, update.message, state);
        await saveState(env, state);
        return OK();
      }
      ...
      if (update.callback_query) {
        await handleCallback(env, env.BOT_TOKEN, update.callback_query, state);
        return OK();
      }
