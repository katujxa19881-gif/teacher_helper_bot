...
function ensureClass(state, cls){
  if(!state.classes[cls]){
    state.classes[cls] = {
      ...
      // единичные актуальные изображения
      schedule_file_id:null, schedule_caption:null, // уроки
      bells_file_id:null,    bells_caption:null,    // звонки
      bus_file_id:null,      bus_caption:null,      // автобусы (город)
      shuttle_file_id:null,  shuttle_caption:null,  // подвоз (школьные)
      // времена забора
...
// КЛЮЧ: не используем \b с кириллицей. Вместо этого — «самодельные границы» (^|[^a-z0-9а-я])
function isBusQuery(t){     return /(^|[^a-z0-9а-я])(расписани[ея].*автобус|автобус|маршрут|городск|муниципал|bus)([^a-z0-9а-я]|$)/.test(t); }
function isShuttleQuery(t){ return /(^|[^a-z0-9а-я])(подвоз|школьн(ый|ик|ые)|шк-?автобус|школ.*автобус)([^a-z0-9а-я]|$)/.test(t); }
...
function resolveTimeNatural(state, msg, freeText, teacherName){
  ...
  if (!mapping) {
    ...
    return { ok:false, text:`Для ${cls} ещё не задано время (${scopeLabel}). Учителю: /pickup_set ${cls} ${scopeLabel} ПН=13:30,ВТ=12:40,...` };
  }
...
async function handleScheduleBusesUpload(env, token, msg, state, cls, caption, file_id){
  ...
  if (/(^|[^a-z0-9а-я])(подвоз|школьн(ый|ик|ые)|шк-?автобус|школ.*автобус)([^a-z0-9а-я]|$)/.test(n)){
  ...
  if (/(^|[^a-z0-9а-я])(расписани[ея].*автобус|автобус|маршрут|городск|муниципал|bus|№\s*\d+)([^a-z0-9а-я]|$)/.test(n)){
  ...
  // ИНАЧЕ — обычное расписание уроков
...
async function handleNaturalMessage(env, token, msg, state){
  ...
  // Конец урока — показать фото звонков
  if (/(когда|во сколько).*(заканчива|конча).*(урок|пара)/.test(t) || /(во сколько звонок|расписани.*звонк)/.test(t)){
  ...
  if (/(во сколько|когда).*(забир|забрать|забирать)/.test(t) || /(до скольки.*(продленк|продлёнк|полдник))/.test(t)){
  ...
  // КАРТА — пополнить (комплект)
  if (/(как.*пополн|пополнить.*карт|пополнение карты|реквизит.*попол)/.test(t)){
    const cls = pickClassFromChat(state, msg.chat.id) || "1Б";
    const items = (state.classes[cls]?.media?.topup || []).slice(0,20);
    if (items.length) await sendMediaItems(token, msg, items);
    return true;
  }
  // КАРТА — баланс (комплект)
  if (/(баланс.*карт|как проверить баланс|проверить.*баланс)/.test(t)){
    const cls = pickClassFromChat(state, msg.chat.id) || "1Б";
  ...
  // УРОКИ — «какие уроки сегодня/завтра/в среду…» + общее «расписание» (без слова автобус/звонок)
  if (/(какие|что за).*(урок|предмет).*(сегодня|завтра|понедельник|вторник|сред|четверг|пятниц|суббот|воскрес)/.test(t)
      || (/расписани/.test(t) && !/автобус|звонк/.test(t))){
    const cls = pickClassFromChat(state, msg.chat.id) || "1Б";
    const rec = state.classes[cls]||{};
  ...
  // ПОДВОЗ (школьные автобусы) — обрабатываем до «автобусов»
  if (isShuttleQuery(t)){
    const cls = pickClassFromChat(state, msg.chat.id) || "1Б";
    const rec = state.classes[cls]||{};
  ...
  // Не знаем — молчим; при включённой пересылке шлём учителю
...
async function cmdPickupSet(env, token, msg, state, args){
  ...
  if (parts[1]){
    ...
    if (/^урок/.test(t2)) scope="main";
    else if (/^(продл|гпд)/.test(t2)) scope="aftercare";
    else if (/^полдник/.test(t2)) scope="snack";
  }

  // 3) хвост — пары ДЕНЬ=ЧЧ:ММ
  const restS = args.trim().slice(args.indexOf(parts[0])+parts[0].length).trim();
  const firstDayIdx = restS.search(/(?:^|[^\S\n,;])(ПН|ВТ|СР|ЧТ|ПТ|СБ|ВС)\s*=/i);
  const tail = (firstDayIdx>=0 ? restS.slice(firstDayIdx) : restS).trim();
  ...
  if(!mapping){
    await sendToSameThread("sendMessage", token, msg, { text:"Формат: /pickup_set 1Б [уроки|продлёнка|полдник] ПН=13:30,ВТ=12:40,..." });
    return true;
  }
  ...
  // оповещение в привязанные чаты
  {
    const rec = state.classes[cls];
    const targets = [rec.general_chat_id, rec.parents_chat_id].filter(Boolean);
    const table = formatPickupWeek(mapping);
...
async function handleCommand(env, token, msg, state){
  ...
  switch(cmd){
    ...
    case "/pickup_set": return await cmdPickupSet(env, token, msg, state, args);
    ...
    case "/persona_set": {
      const isT=state.teacher_id&&state.teacher_id===msg.from.id;
      if(!isT){ await sendToSameThread("sendMessage", token, msg, { text:"Доступ только учителю." }); return true; }
      const name=args.trim(); if(!name){ await sendToSameThread("sendMessage", token, msg, { text:"/persona_set Ирина Владимировна" }); return true; }
    ...
    case "/forward_unknown": {
      const isT=state.teacher_id&&state.teacher_id===msg.from.id;
      if(!isT){ await sendToSameThread("sendMessage", token, msg, { text:"Доступ только учителю." }); return true; }
      const v=(args||"").trim().toLowerCase(); if(!["on","off"].includes(v)){ await sendToSameThread("sendMessage", token, msg, { text:"Используйте: /forward_unknown on|off" }); return true; }
    ...
    case "/media_del": {
      ...
      const topic = (m[0]||"").trim(); const which = (m[1]||"").trim();
      const cls = parseClassFrom(m.slice(2).join(" ")||"");
      if(!topic || !which){ await sendToSameThread("sendMessage", token, msg, { text:"Формат: /media_del <тема> <№|all> [КЛАСС]" }); return true; }
    ...
    case "/media_clear": {
      ...
      if(!isT){ await sendToSameThread("sendMessage", token, msg, { text:"Доступ только учителю." }); return true; }
      const cls = parseClassFrom(args||"");
      clearMedia(state, cls); await saveState(env,state);
    ...
    case "/diag": {
      ...
      const lines = [
        ...
        `• автобусы/подвоз: автобусы:${rec.bus_file_id?"есть ✅":"нет"}  подвоз:${rec.shuttle_file_id?"есть ✅":"нет"}`,
...
export default {
  async fetch(request, env){
    ...
    if (url.pathname===`/webhook/${token}` && request.method==="POST"){
      let update; try{ update = await request.json(); }catch{ return NO(400,"bad json"); }
      ...
      if (update.message && (update.message.photo?.length || update.message.video || update.message.document || update.message.audio || update.message.voice)){
        // только учитель
        if (update.message.chat?.type==="private" && state.teacher_id && state.teacher_id===update.message.from?.id){
          const caption = update.message.caption || "";
          const cls = parseClassFrom(caption||"");
          ensureClass(state, cls);

          let file_id=null, type=null;
          if (update.message.photo?.length){ file_id = extractLargestPhotoId(update.message.photo); type="photo"; }
          else if (update.message.video){ file_id = update.message.video.file_id; type="video"; }
          else if (update.message.document){ file_id = update.message.document.file_id; type="document"; }
          else if (update.message.audio){ file_id = update.message.audio.file_id; type="audio"; }
          else if (update.message.voice){ file_id = update.message.voice.file_id; type="voice"; }
          if (!file_id){ await sendSafe("sendMessage", token, { chat_id: update.message.chat.id, text:"Не удалось распознать вложение." }); return OK(); }

          const n = normalize(caption);

          // копилки: пополнить / баланс
          if (/(попол|пополн|оплат|платеж).*(карт|карта)|карта.*(попол|оплат)/.test(n)){
            pushMedia(state, cls, "topup", { type, file_id, caption });
            await saveState(env, state);
            await sendSafe("sendMessage", token, { chat_id: update.message.chat.id, text:`Сохранено (${cls} — topup).` });
            return OK();
          }
          if (/баланс|остаток.*карт|проверить.*баланс/.test(n)){
            pushMedia(state, cls, "balance", { type, file_id, caption });
            await saveState(env, state);
            await sendSafe("sendMessage", token, { chat_id: update.message.chat.id, text:`Сохранено (${cls} — balance).` });
            return OK();
          }

          // автопубликация расписаний/звонков/автобусов/подвоза
          await handleScheduleBusesUpload(env, token, update.message, state, cls, caption, file_id);
          return OK();
        } else {
          await sendSafe("sendMessage", token, { chat_id: update.message.chat.id, text:"Только учитель может загружать (введите /iam_teacher в личке)." });
          return OK();
        }
      }
